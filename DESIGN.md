...
Vizcom presents a dark, high-contrast digital workbench aesthetic, inspired by traditional drawing and industrial design. The layout features stark monochrome surfaces punctuated by a single vivid logo yellow for primary actions and interactive elements. Thin borders and subtle elevation define components, prioritizing functionality and a focused user experience. Textures of paper, charcoal, and engineering blueprints provide an underlying tactile quality to the otherwise digital interface.
...
| Ideation Blue | `#4586da` | `--color-ideation-blue` | Secondary cool accent for active navigation and interactive highlights. Do not use as the primary CTA color |
...
| Logo Yellow | `#f7cc63` | `--color-clay-orange` | Primary accent for call-to-action buttons and active interactive states, suggesting warmth, momentum, and creation |
...
Filled button with 'Logo Yellow' background (#f7cc63), 'Canvas White' text (#f8f4f1), 8px border-radius, and 12px 18px padding for prominence.
...
- Use 'Logo Yellow' (#f7cc63) exclusively for primary call-to-action buttons and active interactive states, providing a clear visual cue for engagement.
...
This project uses **shadcn/ui** (radix-luma style, taupe base) with **Tailwind CSS v4**. The Vizcom design tokens are mapped to shadcn semantic tokens in `globals.css`. Always use shadcn utility classes — never reference CSS variables directly in component code.

### Token → Tailwind Class Mapping

| Design Token | Tailwind Class | Value |
|---|---|---|
| section-gap (90px) | `py-24` | 96px (closest) |
| card-padding (12px) | `p-3` / `gap-3` | 12px |
| element-gap (8px) | `gap-2` | 8px |
| radius-pill (1440px) | `rounded-full` | pill shape |
| radius-cards (12px) | `rounded-xl` | 12px |
| radius-buttons (8px) | `rounded-lg` | 8px |
| radius-default (8px) | `rounded-lg` | 8px |
| text-caption (12px) | `text-xs` | 12px |
| text-body-sm (14px) | `text-sm` | 14px |
| text-body (16px) | `text-base` | 16px |
| text-body-lg (18px) | `text-lg` | 18px |
| text-heading-sm (20px) | `text-xl` | 20px |
| text-heading (24px) | `text-2xl` | 24px |
| text-heading-lg (56px) | `text-5xl` | 48px (closest) |
| font-weight-regular (400) | `font-normal` | 400 |
| font-weight-medium (500) | `font-medium` | 500 |
| font-weight-bold (700) | `font-bold` | 700 |
| font-matter | `font-heading` | Inter (substitute) |
| font-tomboy-lp | `font-display` | Inter (substitute) |

### Color Mapping (semantic shadcn tokens)

| Role | Light | Dark | Tailwind Class |
|---|---|---|---|
| Background | Canvas White #f8f4f1 | Ink Black #191919 | `bg-background` |
| Foreground | Ink Black #191919 | Canvas White #f8f4f1 | `text-foreground` |
| Primary / CTA | Logo Yellow #f7cc63 | Logo Yellow #f7cc63 | `bg-primary`, `text-primary` |
| Card | Paper Grey #e8e3dd | Dots Black #242425 | `bg-card` |
| Muted | Paper Grey #e8e3dd | Dots Black #242425 | `bg-muted`, `text-muted-foreground` |
| Secondary | Paper Grey #e8e3dd | Dark Card #2f2f31 | `bg-secondary` |
| Border | Button Outline #b5b2b0 | Slate Border #3c3c3e | `border-border` |
| Destructive | Brick Red #c94b3c | Brick Red #c94b3c | `bg-destructive` |

### shadcn Component Usage

| Design Component | shadcn Component | Key Props |
|---|---|---|
| Primary Action Button | `<Button>` | default variant, `rounded-full` for pill |
| Ghost Button | `<Button variant="ghost">` | `rounded-lg` |
| Outline Button | `<Button variant="outline">` | `rounded-lg` |
| Standard Card | `<Card>` | default (uses `bg-card`, `rounded-4xl`) |
| Text Input | `<Input>` | default |
| Status Badge | `<Badge>` | default or outline variant |
| Pill Filter | `<Button variant="ghost" className="rounded-full">` | pill shape |

### Rules

- Always use shadcn Tailwind classes (`bg-muted`, `text-primary`, `rounded-xl`), not raw CSS vars
- Use `font-heading` for section headings, `font-display` for hero/display text
- Use shadcn `Button`, `Card`, `Badge`, `Input`, `Label`, `Textarea`, `Accordion` components
- Section spacing: `py-24` between major sections
- Alternate section backgrounds: default `bg-background` and `bg-muted/50` for contrast

## Agent Prompt Guide

Quick Color Reference (use shadcn classes, not hex):
- text: `text-foreground` / `text-muted-foreground`
- background: `bg-background` / `bg-muted/50`
- border: `border-border`
- accent/CTA: `bg-primary` / `text-primary`

Example Component Prompts:
1. Primary Action Button: `<Button className="rounded-full">` — uses bg-primary (Logo Yellow), rounded pill shape
2. Feature card: `<Card>` with `<CardHeader>`, `<CardContent>` — uses bg-card, rounded-4xl by default. Heading with `font-heading text-2xl font-bold`. Body with `text-sm text-muted-foreground`. Gap with `gap-2`
3. Text input: `<Input placeholder="..." />` — uses bg-input, rounded-3xl by default
